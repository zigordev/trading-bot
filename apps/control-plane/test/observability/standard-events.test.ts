...
test('a start names the release and the runtime, plus what the service adds', async () => {
  const { logServiceStarted } = await import('../../src/observability/standard-events.js');
...
test('a stop names the signal', async () => {
  const { logServiceStopping } = await import('../../src/observability/standard-events.js');
...
test('a failed request is an error with its route, status and stack', async () => {
  const { logRequestFailed } = await import('../../src/observability/standard-events.js');
...
test('a crash is logged without changing how the process dies', async () => {
  const { observeProcessFailures } = await import('../../src/observability/standard-events.js');
