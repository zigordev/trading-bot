...
export type DataReadinessSnapshotEnvelope = {
  ...
  data: {
    items: Array<
      Omit<DataReadinessProjectionInput, "sourceEventId" | "sourceOccurredAt">
    >;
  };
};
...
const isProjectionStatus = (
  value: unknown,
): value is DataReadinessProjectionInput["status"] =>
  value === "ready" ||
  value === "partial" ||
  value === "missing" ||
  value === "error";
...
export const createDataReadinessProjectionConsumer = (
  ...
  const consumer =
    ...
    kafka.consumer({ groupId: config.dataReadinessEventsConsumerGroupId });
