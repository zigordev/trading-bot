...
export const registerOpsRoutes = (
  ...
  const listExecutionTradesFn = options?.listExecutionTradesFn ?? listExecutionTrades;
  ...
  const buildDataReadiness = async (request?: {
    query?: {
      strategyName?: string;
    };
  }) => {
  ...
  const buildExecutionSummary = async () => {
    ...
    const realizedPnlUsd = closedTrades.reduce((sum, item) => sum + (item.realizedPnlUsd ?? 0), 0);
  ...
  app.get('/v1/ops/execution/trades', {
    ...
    handler: async (request) => {
      ...
      return listExecutionTradesFn(pool, {
        ...
        openedTo: typeof query.openedTo === 'string' ? query.openedTo : undefined,
        side: query.side === 'long' || query.side === 'short' ? query.side : undefined,
        status:
          query.status === 'open' ||
          query.status === 'closed' ||
  ...
  app.get('/ws/ops', { websocket: true }, (socket) => {
